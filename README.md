...
4. **Frozen HAR Knowledge Anchor:** Neural corrections are bounded via an inverse-softplus formulation: $\hat{x}_{t,n}^{(h)} = \operatorname{softplus}(\operatorname{softplus}^{-1}(\hat{x}_{t,n}^{\text{HAR},(h)}) + \alpha_n r_{b,n} \delta_{b,n})$ guaranteeing mathematical positivity and anchoring training stability.
