...
4. **Frozen HAR Knowledge Anchor:** Neural corrections are bounded via an inverse-softplus formulation: $ \hat{x}_{t,n}^{(h)} = \mathrm{softplus}\big( \mathrm{softplus}^{-1}(\hat{x}_{t,n}^{\mathrm{HAR},(h)}) + \alpha_n r_{b,n} \delta_{b,n} \big) $ guaranteeing mathematical positivity and anchoring training stability.
